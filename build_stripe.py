#!/usr/bin/env python3
"""Stripe succeeded charges -> per-closer cash collected by day/week/month/allTime.
...
Writes stripe.partial.json (merged into data.json by build_dashboard.py).

Rule (same as sales-leaderboard-dashboard / AIFS CRO dashboard): cash is ALWAYS the Stripe
figure, never GHL's flat close-won stamp. Excludes charges under $1 (card/link
verification charges, not real sales).
"""
import json
import sys
...
def main():
    ...
    print("OK stripe.partial.json written. all-time total cash=$%.2f, unattributed=$%.2f" % (
        window_total_cents["allTime"] / 100.0, unattributed["allTime"]["cash_cents"] / 100.0))
